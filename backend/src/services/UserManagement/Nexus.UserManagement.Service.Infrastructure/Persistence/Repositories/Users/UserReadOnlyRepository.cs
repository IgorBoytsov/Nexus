...
namespace Nexus.UserManagement.Service.Infrastructure.Persistence.Repositories.Users
{
    ...
    {        
        public async Task<Shared.Contracts.UserManagement.Responses.UserAuthDataResponse> GetUserByIdAuth(Guid userId)
        {
            var sql = SqlLoader.Load("Users", "GetUserByIdAuth");
            var user = await connection.QueryFirstOrDefaultAsync<Shared.Contracts.UserManagement.Responses.UserAuthDataResponse>(sql, new { userId });

            return user!;
        }

        public async Task<Shared.Contracts.UserManagement.Responses.UserAuthDataResponse> GetUserByLoginAuth(string login)
        {
            var sql = SqlLoader.Load("Users", "GetUserByLoginAuth");
            var user = await connection.QueryFirstOrDefaultAsync<Shared.Contracts.UserManagement.Responses.UserAuthDataResponse>(sql, new { login });
