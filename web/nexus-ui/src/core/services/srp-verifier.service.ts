...
import { CryptoProfile, CryptoService, KeyDerivationService, SecurityUtils, SrpClientService, SrpContext } from "@crossdyne/security";
...
export class SrpVerifierService {
    ...
    async generateVerifier(
        ...
        ctx: SrpContext, profile: CryptoProfile) : Promise<{ encryptedVerifier: string, encryptedVerifierWrapKeyBase64: string}>{

        const srpAuthHashBytes = await this.keyDerivation.deriveAuthHashForSrp(login, password, srpSalt, ctx.hashAlgorithmName);
        ...
        const encryptedVerifierBase64 = await this.crypto.encryptData(verifierBase64, dekForVerifier, profile.aesGcmOptions);
